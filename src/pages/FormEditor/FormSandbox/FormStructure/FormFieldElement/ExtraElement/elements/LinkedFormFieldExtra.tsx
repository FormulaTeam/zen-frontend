...
function LinkedFormFieldExtra({ extra, onChange, validationErrors, disabled }: Props) {
  ...
  return (
    <>
      <FormControl
        disabled={disabled || !availableForms.length}
        error={!!validationErrors?.properties?.linkedFormId}>
        <InputLabel id="linked-form-label">
          {availableForms.length ? "בחירת טופס" : "אין טפסים זמינים"}
        </InputLabel>
        <Select
          labelId="linked-form-label"
          variant={"standard"}
          ...
          {availableForms.map((availableForm) => (
            <MenuItem key={availableForm.id} value={availableForm.id}>
              {availableForm.title}
            </MenuItem>
          ))}
        </Select>
